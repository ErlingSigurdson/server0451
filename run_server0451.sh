...
if [ "$TRACE" != "$VALID_TRACE_1" ] && [ "$TRACE" != "$VALID_TRACE_2" ]; then
    echo -e "Valid status for CLOSE-WAIT-tracing script is not specified in the config file."
...
if [ "$TRACE" = "on" ]; then
    nohup $TRACE_SCRIPT_FILE_PATH\
          $EXEC_BIN_FILE_NAME $RUN_SCRIPT_FILE_NAME $LOG_FLUSH_SCRIPT_FILE_PATH $RUN_SCRIPT_FILE_PATH $LOG_FILE_PATH\
          >> $LOG_FILE_PATH 2>&1 &
fi
